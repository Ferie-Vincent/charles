      ...
      {sorted.map(p => {
        ...
        return (
          <Link key={p.id} to={`/projects/${p.id}`} className="tl-card-v2">
            ...
            <div className="tl-card-v2__footer">
              <div className="tl-card-v2__delay" style={{ color: delayColor }}>
                {delayText}
              </div>
              <div className={`tl-card-v2__docs${p.doc_count === 0 ? ' tl-card-v2__docs--empty' : ''}`} title={p.doc_count === 0 ? 'Aucun document' : `${p.doc_count} document${p.doc_count > 1 ? 's' : ''}`}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="11" height="11">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                  <polyline points="14 2 14 8 20 8"/>
                </svg>
                {p.doc_count > 0 ? p.doc_count : '—'}
              </div>
            </div>
          </Link>
        );
