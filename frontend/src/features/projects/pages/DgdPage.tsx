...
export default function DgdPage() {
  ...
  const initMut = useMutation({ mutationFn: () => initializeDgd(projectId), onSuccess: invalidate });
  const signEntrepriseMut = useMutation({ mutationFn: () => signDgd(projectId, 'entreprise', signDate), onSuccess: invalidate });
  const signMoaMut = useMutation({ mutationFn: () => signDgd(projectId, 'moa', signDate), onSuccess: invalidate });
  const obsMut = useMutation({
    mutationFn: () => updateDgdObservations(projectId, observations),
    onSuccess: () => { invalidate(); setEditingObs(false); },
  });
          ...
          {canManage ? (
            <>
              <p style={{ color: 'var(--color-text-muted)', marginBottom: '1rem' }}>
                Aucun DGD initialisé. Disponible une fois le chantier en phase Réception.
              </p>
              <button className="btn btn--primary" onClick={() => initMut.mutate()} disabled={initMut.isPending}>
            ...
            <p style={{ color: 'var(--color-text-muted)' }}>Aucun DGD disponible pour ce chantier.</p>
          )}
        </div>
      </div>
    );
  ...
  const rows = [
    ...
    { label: 'Total situations certifiées (HT)', value: dgd.total_situations_ht },
    ...
    ...(dgd.avance_remboursement_restant > 0
      ? [{ label: 'Avance démarrage non remboursée (−)', value: -dgd.avance_remboursement_restant, warn: true }]
      : []),
    ...
    <div className="page-container">
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>Observations</h2>
          {canManage && dgd.status !== 'signe_moa' && !editingObs && (
            <button className="btn btn--ghost btn--sm" onClick={() => { setObservations(dgd.observations ?? ''); setEditingObs(true); }}>
              Modifier
            </button>
          )}
        </div>
        {editingObs ? (
          <div>
            <textarea
              ...
              placeholder="Mentions particulières, réserves levées, litiges résolus…"
              style={{ width: '100%', marginBottom: '0.75rem', resize: 'vertical' }}
            />
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn--primary btn--sm" onClick={() => obsMut.mutate()} disabled={obsMut.isPending}>
                {obsMut.isPending ? 'Enregistrement…' : 'Enregistrer'}
              </button>
              <button className="btn btn--ghost btn--sm" onClick={() => setEditingObs(false)}>Annuler</button>
            </div>
          </div>
        ) : (
          <p style={{ color: dgd.observations ? 'inherit' : 'var(--color-text-muted)', fontSize: '14px', margin: 0, whiteSpace: 'pre-wrap' }}>
            {dgd.observations ?? 'Aucune observation.'}
          </p>
        )}
      </div>

      {/* Signatures */}
      {canManage && dgd.status !== 'signe_moa' && (
        <div className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <div className="form-group" style={{ margin: 0 }}>
              ...
              <input
                type="date" className="form-control" value={signDate}
                onChange={e => setSignDate(e.target.value)} style={{ width: '180px' }}
              />
            </div>
            {dgd.status === 'brouillon' && (
              <button className="btn btn--primary" onClick={() => signEntrepriseMut.mutate()} disabled={signEntrepriseMut.isPending} style={{ alignSelf: 'flex-end' }}>
            ...
            {dgd.status === 'signe_entreprise' && (
              <>
                ...
                <button className="btn btn--primary" onClick={() => signMoaMut.mutate()} disabled={signMoaMut.isPending} style={{ alignSelf: 'flex-end' }}>
